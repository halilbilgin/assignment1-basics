...
def run_embedding(
    ...
    embedding = Embedding(num_embeddings=vocab_size, embedding_dim=d_model, dtype=torch.float)
...
def run_scaled_dot_product_attention(
    ...
    """
    return ScaledDotProductAttention(Q.shape[-1])(Q,K,V,mask)
...
def run_multihead_self_attention_with_rope(
    ...
    return multi_head_self_attention(in_features, token_positions=token_positions)
...
def run_transformer_block(
    ...
    transformer.multi_head_self_attention.load_state_dict(
        {
    ...
    transformer.rms_norm_first.load_state_dict(
        {"g": weights["ln1.weight"]}
    )
    transformer.feed_forward.load_state_dict(
        {
    ...
    transformer.rms_norm_second.load_state_dict(
...
def run_transformer_lm(
    ...
    for block_index in range(num_layers):
        transformer = transformer_lm.transformer_blocks[block_index]
        transformer.multi_head_self_attention.load_state_dict(
            {
                "q_proj_weight": weights[f"layers.{block_index}.attn.q_proj.weight"],
                "k_proj_weight": weights[f"layers.{block_index}.attn.k_proj.weight"],
                "v_proj_weight": weights[f"layers.{block_index}.attn.v_proj.weight"],
                "o_proj_weight": weights[f"layers.{block_index}.attn.output_proj.weight"]
            }
        )
        transformer.rms_norm_first.load_state_dict(
            {"g": weights[f"layers.{block_index}.ln1.weight"]}
        )
        transformer.feed_forward.load_state_dict(
            {
                "W1": weights[f"layers.{block_index}.ffn.w1.weight"],
                "W2": weights[f"layers.{block_index}.ffn.w2.weight"],
                "W3": weights[f"layers.{block_index}.ffn.w3.weight"],
            }
        )
        transformer.rms_norm_second.load_state_dict(
            {"g": weights[f"layers.{block_index}.ln2.weight"]}
        )
    
    transformer_lm.rms_norm_final.load_state_dict(
        {"g": weights["ln_final.weight"]}
    )

    transformer_lm.final_linear.load_state_dict({"weight": weights["lm_head.weight"]})
...
def run_rmsnorm(
    ...
    """
    rms_norm = RMSNorm(d_model=d_model, eps=eps, dtype=torch.float32)
...
def run_softmax(in_features: Float[Tensor, " ..."], dim: int) -> Float[Tensor, " ..."]:
    ...
    """
    return SoftMax(dim)(in_features)
