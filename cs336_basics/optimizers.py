...
class CrossEntropy(nn.Module):
    ...
    def forward(self, inputs: Float[Tensor, "batch_size output_size"], targets: Int[Tensor, " batch_size"]) -> Float[Tensor, ""]:
    ...
    def step(self, closure: Optional[Callable[[], float]] = None) -> Optional[float]:
        ...
        for group in self.param_groups:
            ...
            for p in group["params"]:
                ...
                state["t"] = t+1
                current_lr = lr*math.sqrt(1-beta1**t)/(1-(beta0**t))
                p.data -= current_lr*state["m"]/(torch.sqrt(state["v"])+eps)
...
def learning_rate_scheduler(it: int,
